...
--[[
The following awards aren't actually fully coded in yet. Instead this is pseudocode that kinda explains what I feel like I need to do in order to make it work.

local function SurvivedMultipleRounds(events, scores, players, traitors)
   local most = SurvivedMostRounds(events, ROUNDS)
   if not most then return nil end

   local nick = players[most.sid]
   if not nick then return nil end

   local award = {nick=nick, priority=most.rounds}
   local role = nick.role
   local rounds = nick.roundsSurvived
   if role = traitor then
      if rounds > 3 and rounds < 5 then
         award.title = T("aw_surt1_title")
         award.text = PT("aw_surt1_text", {num = rounds})
      elseif rounds > 5 then
	     award.title = T("aw_surt2_title")
         award.text = PT("aw_surt2_text", {num = rounds})
	  else 
	     return nil 
	  end
   else then
      if rounds > 3 and rounds < 5 then
         award.title = T("aw_suri1_title")
         award.text = PT("aw_suri1_text", {num = rounds})
      elseif rounds > 5 then
	     award.title = T("aw_suri2_title")
         award.text = PT("aw_suri2_text", {num = rounds})
	  else 
	     return nil 
	  end
   end
...
local function DiedMultipleRounds(events, scores, players, traitors)
   local most = DiedMostRounds(events, ROUNDS)
   if not most then return nil end
   ...
   local award = {nick=nick, priority=most.rounds}
   local role = nick.role
   local rounds = nick.roundsDied
   if role = traitor then
      if rounds > 3 and rounds < 5 then
         award.title = T("aw_bdlkt1_title")
         award.text = PT("aw_bdlkt1_text", {num = rounds})
      elseif rounds > 5 then
	     award.title = T("aw_bdlkt2_title")
         award.text = PT("aw_bdlkt2_text", {num = rounds})
	  else 
	     return nil 
	  end
   else then
      if rounds > 3 and rounds < 5 then
         award.title = T("aw_bdlki1_title")
         award.text = PT("aw_bdlki1_text", {num = rounds})
      elseif rounds > 5 then
	     award.title = T("aw_bdlki2_title")
         award.text = PT("aw_bdlki2_text", {num = rounds})
	  else 
	     return nil 
	  end
   end
...
local function DNAKills(events, scores, players, traitors)
   local most = KilledMostWithDNA(events, ROUNDS)
   if not most then return nil end
   ...
   local award = {nick=nick, priority=most.kills}
   local role = nick.role
   local kills = nick.kills
   if kills > 1 and kills < 2 then
      award.title = T("aw_dna1_title")
      award.text = PT("aw_dna1_text")
   elseif kills > 2 then
	  award.title = T("aw_dna2_title")
      award.text = PT("aw_dna2_text", {num = kills})
   else 
	  return nil 
   end
